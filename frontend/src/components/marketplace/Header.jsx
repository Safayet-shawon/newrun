...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-nexora-border bg-white/95 backdrop-blur-xl">
      <div className="hidden border-b border-[#DCE9F2] bg-gradient-to-r from-[#ECF7F2] via-[#F6FAFD] to-[#EAF2FB] sm:block">
      ...
      <div className="nx-container flex h-[68px] items-center gap-2 sm:gap-3 lg:gap-4">
        <button
        ...
        <nav className="hidden shrink-0 items-center gap-1 xl:flex">
          <Link to="/shops" className="flex h-10 items-center gap-1.5 rounded-xl px-2.5 text-xs font-semibold text-nexora-ink hover:bg-[#EAF2FB]"><Store size={17} /> Shops</Link>
      ...
      <div className="border-t border-nexora-border bg-white px-3 py-2 md:hidden">
        <SmartSearch mobile />
      </div>
          ...
          <aside className="absolute left-0 top-0 h-[100dvh] w-[310px] max-w-[90vw] overflow-y-auto border-r border-nexora-border bg-white p-4 shadow-2xl">
            ...
            <nav className="mt-4 space-y-1">
              {menuLinks.map(({ label, to, icon: Icon, accent }) => {
                ...
                return (
                  <Link key={label} to={to} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-nexora-mintbg text-nexora-emeraldDark" : accent ? "text-nexora-coral hover:bg-[#FFF4EE]" : "text-nexora-ink hover:bg-[#F1F6FA]"}`}>
                    <Icon size={18} /> {label}
                  </Link>
                );
              })}
            ...
            <div className="mt-5 rounded-2xl border border-[#CDEFE2] bg-gradient-to-br from-[#E8F7F0] to-[#EAF2FB] p-4">
