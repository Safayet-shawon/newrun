  ...
  return (
    <div className="bg-nexora-warm">
      <div className="nx-container pt-3 sm:pt-4">
        <section className="overflow-hidden rounded-3xl border border-nexora-border bg-white shadow-sm">
          <div className="grid min-h-[238px] lg:grid-cols-[1.08fr_1fr_270px]">
            <div className="flex flex-col justify-center px-5 py-6 sm:px-7 lg:px-8">
              <span className="mb-2 inline-flex w-fit items-center rounded-full bg-nexora-mintbg px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-nexora-emeraldDark">Bangladesh&apos;s trusted multi-vendor marketplace</span>
              <h1 className="max-w-xl text-[34px] font-extrabold leading-[1.02] tracking-[-0.03em] text-nexora-ink sm:text-[42px] lg:text-[44px]">Everything you love.<br /><span className="text-nexora-emerald">From stores you can trust.</span></h1>
              ...
              <div className="mt-4 flex flex-wrap gap-2"><Link to="/products" className="nx-btn-primary">Explore Marketplace <ArrowRight size={15} /></Link><Link to="/shops" className="nx-btn-ghost">Discover Shops</Link></div>
              <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-semibold text-nexora-muted"><span className="inline-flex items-center gap-1.5"><Truck size={15} className="text-nexora-emerald" /> Fast delivery</span><span className="inline-flex items-center gap-1.5"><BadgeCheck size={15} className="text-nexora-emerald" /> Verified sellers</span><span className="inline-flex items-center gap-1.5"><ShieldCheck size={15} className="text-nexora-emerald" /> Easy returns</span></div>
            </div>
            ...
            <div className="relative hidden min-h-[238px] overflow-hidden bg-gradient-to-br from-[#F6FBF8] to-[#E9F8F1] sm:block">
              ...
              <div className="absolute inset-4 grid grid-cols-6 grid-rows-5 gap-2">
                <Link to={heroTiles[0].to} aria-label={`Shop ${heroTiles[0].label}`} className="group col-span-3 row-span-4 overflow-hidden rounded-3xl bg-white shadow-sm"><img src={heroTiles[0].image} alt={heroTiles[0].label} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /></Link>
                <Link to={heroTiles[1].to} aria-label={`Shop ${heroTiles[1].label}`} className="group col-span-3 row-span-2 overflow-hidden rounded-3xl bg-white shadow-sm"><img src={heroTiles[1].image} alt={heroTiles[1].label} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /></Link>
                ...
                <div className="col-span-6 row-span-1 flex items-center justify-between rounded-2xl bg-white/90 px-4 text-xs font-bold text-nexora-ink shadow-sm backdrop-blur"><span>Fashion · Tech · Beauty · Home · More</span><Link to="/products" className="text-nexora-emerald hover:text-nexora-emeraldDark">One marketplace →</Link></div>
              </div>
            </div>
        ...
        <section className="mt-6 grid gap-6 xl:grid-cols-[1.6fr_1fr]"><div><SectionTitle title="Trending Now" subtitle="Most-loved products from shops across Nexora" to="/products?sort=popular" /><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-3 2xl:grid-cols-6">{trending.map((product, index) => <ProductCard key={product.id} product={product} index={index} />)}</div></div><div><SectionTitle title="Shops You’ll Love" subtitle="Independent stores worth discovering" to="/shops" /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{featuredShops.map((shop) => <ShopCard key={shop.id} shop={shop} />)}</div></div></section>

        <section className="mt-8 rounded-3xl border border-[#D7EEE4] bg-[#F4FBF7] p-4 sm:p-6"><SectionTitle title="Men’s Picks" subtitle="Two rows of fashion, footwear and everyday favourites" to="/products?category=fashion&q=men" link="Shop men" /><ProductGrid items={mensPicks} limit={12} /></section>
        <section className="mt-6 rounded-3xl border border-[#F2DED5] bg-[#FFF9F6] p-4 sm:p-6"><SectionTitle title="Women’s Picks" subtitle="Two rows of fashion, beauty and accessories to explore" to="/products?category=fashion&q=women" link="Shop women" /><ProductGrid items={womensPicks} limit={12} /></section>
        ...
        {!!data?.brands?.length && <section className="mt-8 rounded-3xl border border-nexora-border bg-white p-5 shadow-sm"><SectionTitle title="Brands to Know" subtitle="Discover brands available across Nexora" to="/products" /><div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">{data.brands.slice(0, 14).map((brand, index) => { const name = typeof brand === "string" ? brand : brand.name; return <Link key={`${name}-${index}`} to={`/products?brand=${encodeURIComponent(name)}`} className="min-w-[140px] rounded-2xl border border-nexora-border bg-[#FBFDFC] px-5 py-4 text-center text-sm font-extrabold text-nexora-ink transition hover:border-nexora-emerald hover:text-nexora-emerald">{name}</Link>; })}</div></section>}

        <section className="my-8 grid gap-5 overflow-hidden rounded-3xl border border-[#CDEFE2] bg-gradient-to-r from-[#E9F8F1] via-[#F7FCF9] to-[#FFF7E8] p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><span className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-nexora-emerald">Build with Nexora</span><h2 className="mt-1 text-2xl font-extrabold text-nexora-ink sm:text-3xl">Your shop. Your identity. One trusted marketplace.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-nexora-muted">Create a storefront, publish products and reach shoppers across Bangladesh without losing your own brand identity.</p></div><div className="flex flex-wrap gap-2"><Link to="/seller/signup" className="nx-btn-primary">Become a Seller <ArrowRight size={15} /></Link><Link to="/shops" className="nx-btn-ghost">Discover Shops</Link></div></section>
      </div>
    </div>
  );
}
