...
export default function ForgotPassword() {
  ...
  return (
    <AuthShell title="Reset your password" subtitle="We'll send you a secure link to get back into your account.">
      {sent ? (
        <div className="rounded-2xl border border-nexora-border bg-nexora-mintbg p-5 text-sm text-nexora-ink" data-testid="reset-sent">
          If an account exists for <span className="font-semibold">{email}</span>, a password reset link has been sent. Please check your inbox and spam folder.
        </div>
      ) : (
        <form onSubmit={submit} className="space-y-4" data-testid="forgot-form">
          ...
          {error && <p className="text-sm text-nexora-coral" role="alert">{error}</p>}
          <button disabled={loading} className="nx-btn-primary w-full" data-testid="forgot-submit">{loading ? "Sending…" : "Send reset link"}</button>
        </form>
      )}
