"""Atomic COD order creation. A MongoDB replica set is required for checkout."""
import os
import json
import hashlib
...
from product_rules import resolve_selection, paisa
...
async def build_quote(body, user, session=None):
    ...
    if any(not str(address.get(k," ")).strip() for k in ("full_name","phone","address","city")):
        raise HTTPException(422,"Complete your delivery address")
    grouped = {}
    ...
    for item in body.items:
        ...
        if seller and seller.get("status") == "suspended":
        ...
        inventory[p["id"]] = p
        group = grouped.setdefault(shop["id"],{"shop_id":shop["id"],"shop_name":shop["name"],"shop_slug":shop["slug"],"seller_id":shop["seller_id"],"items":[],"subtotal_paisa":0})
        group["items"].append({"product_id":p["id"],"title":p["title"],"qty":item.qty,"price":unit/100,"unit_price_paisa":unit,"options":options,"customization":custom,"variant": " / ".join(f"{k}: {v}" for k,v in options.items()),"product_type":p.get("product_type","general")})
        group["subtotal_paisa"] += unit*item.qty
    subtotal = sum(g["subtotal_paisa"] for g in grouped.values())
    # Preserve the existing delivery rule, now evaluated and persisted by the server.
    shipping = 0 if subtotal > paisa(os.getenv("FREE_DELIVERY_OVER_BDT","2000")) else paisa(os.getenv("DELIVERY_FEE_BDT","60"))
    for index, group in enumerate(grouped.values()):
        group["delivery_paisa"] = shipping if index == 0 else 0
        group["total_paisa"] = group["subtotal_paisa"]+group["delivery_paisa"]
        group["total"] = group["total_paisa"]/100
    quote = {"groups":list(grouped.values()),"subtotal_paisa":subtotal,"delivery_paisa":shipping,"total_paisa":subtotal+shipping,"payment_method":"cash_on_delivery"}
...
async def quote_order(body: CheckoutBody,user:dict=Depends(get_current_user)):
    ...
    quote["payment_method"] = body.payment_method
    return quote
...
async def checkout(body: CheckoutBody,user:dict=Depends(get_current_user)):
    ...
    async def commit(session):
        ...
        if body.expected_total_paisa is None or quote["total_paisa"] != body.expected_total_paisa:
            raise HTTPException(409,"Prices changed or the order was not reviewed. Refresh the order quote.")
        created = []
        ...
        for product_id,qty in stocks.items():
            ...
            # Match the option catalogue snapshot to guard against concurrent seller edits.
            p = inventory[product_id]
            condition["price"] = p["price"]
            condition["discount_price"] = p.get("discount_price")
            if "variants" in p: condition["variants"] = p["variants"]
            if "variant_inventory" in p: condition["variant_inventory"] = p["variant_inventory"]
        ...
        for group in quote["groups"]:
            order = {**group,"id":new_id("ord_"),"checkout_id":checkout_id,"customer_id":user["id"],"customer_name":user["name"],"customer_email":user["email"],"delivery_address":{k:v for k,v in address.items() if k not in ("user_id","_id")},"status":"pending","payment_status":"unpaid","payment_method":"cash_on_delivery","commission_status":"policy_pending","commission_rate":None,"created_at":now_iso()}
            order["payment_method"] = body.payment_method
            order["payment_status"] = "paid" if body.payment_method == "nexora_wallet" else "unpaid"
            if body.payment_method == "nexora_wallet":
                order["payment_mode"] = wallet_mode()
            await db.orders.insert_one(dict(order),session=session)
            created.append(order)
        await db.checkouts.insert_one({**identity,"digest":digest,"orders":created,"created_at":now_iso()},session=session)
